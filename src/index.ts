...
app.post("/users", async (req: Request, res: Response) => {
    try {
        ...
        if (id.length < 4) {
            res.status(400)
            throw new Error("id deve possuir dois caracteres")
        }
        ...
        if (name.length < 4) {
            res.status(400)
            throw new Error("name deve possuir dois caracteres")
        }
...
app.delete("/tasks/:taskId/users/:userId", async (req: Request, res: Response) => {
    try {
        ...
        await db("users_tasks").delete().where({task_id:taskIdToDelete}).andWhere({user_id:userIdToDelete})
